...
if [[ $(pactl list sinks short | grep -e 'alsa_output.pci-0000_00_1f.3.analog-stereo' | grep -oE '[A-Z]+$') = "RUNNING" ]]; then
  ...
  if pactl info | grep -ie 'Default Sink' | grep -e 'alsa_output.pci-0000_00_1f.3.analog-stereo' > /dev/null; then
    echo -ne "%{F#AFAFAF} "
  else
    echo -ne "%{F#595959}ﯥ "
  fi
...
if [[ $(pactl list sinks short | grep -e 'alsa_output.usb-Creative_Technology_Ltd_Sound_Blaster_Play__3_00080951-00.analog-stereo' | grep -oE '[A-Z]+$') = "RUNNING" ]]; then
  ...
  if pactl info | grep -ie 'Default Sink' | grep -e 'alsa_output.usb-Creative_Technology_Ltd_Sound_Blaster_Play__3_00080951-00.analog-stereo' > /dev/null; then
    echo -ne "%{F#AFAFAF}"
  else
    echo -ne "%{F#595959}ﳌ"
  fi
